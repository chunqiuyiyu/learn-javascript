...
class FormUI
{
	...
	public function initialHTML ($hashover_wrapper = true)
	{
		...
		$main_inputs->appendChild ($main_post_button);
		$main_form->appendChild ($main_inputs);
		...
		$count_element = new HTMLTag ('h4', array (
			'id' => 'hashover-count'
		));
		...
		// Add comment count and sort dropdown menu wrapper to comments sections
		// $comments_section->appendChild ($count_sort_wrapper);
		$form_section->appendChild ($count_sort_wrapper);
		$form_section->appendChild ($message_container);
